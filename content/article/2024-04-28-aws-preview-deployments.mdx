...
We've been using [S3 sync action](https://github.com/jakejarvis/s3-sync-action) for a while which is very simple and great! Unfortunately, it doesn't support preview deployments out of the box. So, we need to do a little bit of work to make it work. Of course, you can use [AWS CLI](https://aws.amazon.com/cli/) directly or any other action to do the same job. All of them will work, but you need to have infrastaructure ready before deploying any textChangeRangeNewSpan.
...
- Cloudfront fucntions: to add basic authentication to our preview deployments and redirects
- Route 53: to manage our DNS records

Here is how our architecture will look like:
...
I hope this could be helpful for a lot of people that are not looking to migrate from AWS and would like to have preview deployments as part of their developer experience workflow. I have created an open source GitHub Action that you can use to deploy your websites to AWS S3 with preview deployments.
...
The pipeline will run on every Pull request and deploy the changes to the preview bucket. The preview bucket will be created automatically if it doesn't exist. The preview URL will be added to the Pull request as a Github deployment. If everything goes well, here is the screenshot where you can find a link for preview deployment:
...
There are a few ideas that I already have in mind and planning to add in the near future:
...
- Github comments with progress and more details about deployments. I think GitHub environment is a great place to start but they are quite limited.
- Add Docker support. The action now only deploys static files to S3. Having Docker based preview deployments would be game changer for some projects when people use small Python, Ruby or Next.js apps.
